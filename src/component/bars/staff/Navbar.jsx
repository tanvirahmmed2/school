...
const Navbar = () => {
  ...
  useEffect(() => {
    const fetchStaffProfile = async () => {
      try {
        ...
        console.error('Failed to fetch staff profile in navbar:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 h-16 bg-white border-b border-slate-100 flex items-center justify-between px-4 md:px-6 z-50">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-slate-50 text-slate-600 text-xs font-semibold">
          ...
          {loading ? (
            ...
            <span>{staff ? staff.name : 'Staff'}</span>
          )}
        </div>
