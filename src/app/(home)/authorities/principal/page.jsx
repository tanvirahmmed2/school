...
const PrincipalPage = () => {
    ...
    const fetchPrincipal = async () => {
      try {
        ...
        if (res.ok) {
          ...
          const found = list.find(m => m.designation === 'principal');
          if (found) {
            setMember(found);
          }
        }
  ...
  const displayMember = member || {
    name: 'Prof. Dr. Rahman',
    bio: 'Our institution has always stood at the forefront of quality technical education. Our mandate is simple: to equip students with hands-on skillsets and the mental agility required to thrive in a highly competitive digital market.\n\nThrough rigorous curriculum vetting, research integrations, and direct community activities, we ensure that every student who leaves FIT carries with them a strong moral compass and premium industrial capability.',
    email: 'principal@fit.edu.bd',
    contact: '+880 180 500 0300',
    image: null
  };

  const getInitials = (name) => {
    return name ? name.split(' ').map(n => n[0]).join('').slice(0, 2).toUpperCase() : 'PR';
  };
