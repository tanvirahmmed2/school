...
export async function GET() {
  try {
    ...
    const result = await query(`
      SELECT id, name, email, number, designation, address, role, is_active, is_registered
      FROM staff
      WHERE id = $1 AND is_active = TRUE AND is_registered = TRUE
    `, [decoded.id]);

    if (result.rows.length === 0) {
      return NextResponse.json({ error: 'Staff account is inactive or not found' }, { status: 404 });
    }
