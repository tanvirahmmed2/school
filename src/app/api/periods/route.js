...
export async function GET() {
  try {
    ...
    const res_err = { error: 'Failed to retrieve academic periods.' };
    return NextResponse.json({
      success: false,
...
// POST create academic period (Admin only)
export async function POST(request) {
  try {
    ...
    const res_err = { error: 'Failed to create academic period. Duplicate names/times might have occurred.' };
    return NextResponse.json({
      success: false,
      message: res_err.error,
      error: res_err.error,
