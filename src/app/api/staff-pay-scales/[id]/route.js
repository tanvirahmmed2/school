...
// PUT update a staff pay scale grade (Admin only)
export async function PUT(request, { params }) {
  try {
    ...
    const duplicateCheck = await query(
      'SELECT id FROM staff_pay_scale WHERE LOWER(name) = LOWER($1) AND id <> $2',
      [name.trim(), id]
    );
    ...
    if (updatedGrade.rowCount === 0) {
      return NextResponse.json({
        success: false,
        message: 'Staff pay scale grade not found.',
        error: 'Not Found',
        paylod: null
      }, { status: 404 });
    }
    ...
    const res_data = {
      message: 'Staff pay scale grade updated successfully.',
    ...
    console.error('Error updating staff pay scale:', error);
    return NextResponse.json({
      success: false,
      message: 'Failed to update staff pay scale. Internal server error.',
...
// DELETE a staff pay scale grade (Admin only)
export async function DELETE(request, { params }) {
  try {
    ...
    const deleteResult = await query('DELETE FROM staff_pay_scale WHERE id = $1 RETURNING id', [id]);
    ...
    const res_data = {
      message: 'Staff pay scale grade deleted successfully.',
      paylod: { id }
    };
    return NextResponse.json({
      success: true,
    ...
    console.error('Error deleting staff pay scale:', error);
    return NextResponse.json({
      success: false,
      message: 'Failed to delete staff pay scale. Internal server error.',
      error: 'Internal Server Error',
      paylod: null
    }, { status: 500 });
  }
