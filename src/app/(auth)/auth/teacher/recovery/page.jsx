...
const TeacherRecovery = () => {
  ...
  const handleRequestToken = async (e) => {
    ...
    try {
      ...
      toast.success(data.message || 'Verification token sent to your email successfully.');
      setStep(2);
    } catch (err) {
      ...
      <div className="w-full max-w-110 animate-fade-up z-10">
        <div className="flex flex-col items-center mb-8 text-center">
          ...
          <p className="text-sm text-slate-500 max-w-80">
            {step === 1 
              ? 'Enter your registered teacher email to request a 6-digit password recovery code.' 
          ...
          {step === 1 ? (
            ...
            <form onSubmit={handleResetPassword} className="w-full flex flex-col gap-5">
              ...
              <button
                ...
                onClick={() => setStep(1)}
                disabled={loading}
                className="w-full text-center text-xs font-semibold text-slate-455 hover:text-slate-600 transition-colors mt-1 underline cursor-pointer"
              >
        ...
        <div className="w-full text-center mt-6 flex justify-center gap-4">
          <Link
            ...
            className="text-xs font-semibold text-indigo-600 hover:text-indigo-700 transition-colors py-1 px-3 rounded-full hover:bg-indigo-50"
          >
            Back to Login
          </Link>
          <Link
