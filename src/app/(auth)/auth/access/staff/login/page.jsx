...
import { FiLock, FiMail, FiArrowRight, FiSliders, FiHome, FiShield, FiRefreshCw, FiArrowLeft } from 'react-icons/fi';
...
const StaffLogin = () => {
  ...
  const handleOtpSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const response = await fetch('/api/staff/verify-2fa', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleResendCode = async () => {
    ...
    setResending(true);
    try {
      const response = await fetch('/api/staff/resend-2fa', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    <div className="w-full min-h-screen flex flex-col items-center justify-center bg-slate-50 text-slate-900 relative px-4 py-12 overflow-hidden">
      ...
      <div className="w-full max-w-110 animate-fade-up z-10">
        <div className="flex flex-col items-center mb-8 text-center">
          ...
          <p className="text-sm text-slate-500 max-w-80">Log in to access Cashier, Registrar, and support desks.</p>
        </div>
        ...
        <div className="w-full bg-white border border-slate-100 rounded-3xl p-8 shadow-[0_10px_40px_rgba(0,0,0,0.03)]">
          {step === '2fa' ? (
            <form onSubmit={handleOtpSubmit} className="w-full flex flex-col gap-5 animate-fade-up">
              <div className="flex flex-col items-center text-center gap-2 p-4 bg-sky-50 border border-sky-200/60 rounded-2xl">
                <FiShield className="text-2xl text-sky-600 mb-1" />
              ...
              <div className="flex flex-col gap-2">
                ...
                <input
                  ...
                  disabled={loading}
                  placeholder="• • • • • •"
                  className="w-full text-center px-4 py-3 bg-white border border-slate-300 rounded-xl text-xl font-bold tracking-[0.4em] text-slate-900 placeholder:text-slate-300 outline-none transition-all duration-200 focus:border-sky-600 focus:ring-4 focus:ring-sky-500/10"
                />
              </div>
              ...
              <div className="flex items-center justify-between pt-2 border-t border-slate-100 text-xs">
                ...
                <button
                  ...
                  disabled={resending}
                  className="flex items-center gap-1.5 font-semibold text-sky-600 hover:text-sky-700 transition-colors disabled:opacity-50 cursor-pointer"
                >
            ...
            <form onSubmit={handleCredentialsSubmit} className="w-full flex flex-col gap-5">
              ...
              <div className="flex flex-col gap-2">
                <div className="flex justify-between items-center">
                  ...
                  <Link
                    href="/auth/access/staff/recovery"
              ...
              >
                {loading ? (
                  ...
                  <>
                    <FiSliders className="text-lg" /> Staff Login <FiArrowRight className="text-lg" />
                  </>
                )}
              </button>
