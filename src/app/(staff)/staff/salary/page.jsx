...
const StaffSalaryHistoryPage = () => {
  ...
  useEffect(() => {
    const fetchSalary = async () => {
      try {
        ...
        console.error('Failed to load staff salary ledger:', err);
      } finally {
        setLoading(false);
      }
    };
  ...
  const stats = [
    ...
    { label: 'Pending Credits', value: totalPending, color: totalPending > 0 ? 'bg-amber-50 text-amber-600 border-amber-100' : 'bg-slate-50 text-slate-550 border-slate-100', icon: FiClock }
  ];
    ...
    <div className="flex flex-col gap-8 w-full max-w-6xl mx-auto">
      ...
      <div className="bg-white border border-slate-100 rounded-3xl p-6 md:p-8">
        <h2 className="text-base font-bold text-slate-800 mb-6 flex items-center gap-2">
          <FiDollarSign className="text-indigo-650 text-indigo-600" /> Salary History
        </h2>
...
export default StaffSalaryHistoryPage;
