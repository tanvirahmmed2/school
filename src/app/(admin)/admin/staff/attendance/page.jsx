...
const AdminStaffAttendancePage = () => {
  ...
  const [monthlyData, setMonthlyData] = useState({ staffList: [], attendanceLogs: [] });
  const [loading, setLoading] = useState(true);

  const fetchMonthlySheet = async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/admin/staff/attendance?month=${selectedMonth}`);
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || 'Failed to load attendance.');
      setMonthlyData({
        staffList: data.paylod?.staffList || [],
        attendanceLogs: data.paylod?.attendanceLogs || []
      });
    } catch (err) {
  ...
  const handleExportMonthlyXLSX = () => {
    if (monthlyData.staffList.length === 0) {
      toast.error('No records to export.');
      return;
    }

    const rows = monthlyData.staffList.map((staff) => {
      const staffLogs = monthlyData.attendanceLogs.filter((l) => l.staff_id === staff.staff_id);
    ...
    XLSX.writeFile(workbook, `Staff_Attendance_${selectedMonth}.xlsx`);
  ...
  return (
    <div className="w-full max-w-7xl mx-auto space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 pb-3 border-b border-slate-200">
        <div>
          <h1 className="text-lg font-bold text-slate-800 tracking-tight flex items-center gap-2">
            <FiCalendar className="text-primary" /> Staff Monthly Attendance
          </h1>
          <p className="text-xs text-slate-500 mt-0.5">
            Monthly sheet matrix for staff attendance.
          </p>
        </div>

        <div className="flex items-center gap-2">
          <Link
            href="/admin/staff/attendance/record"
          ...
          <button
            ...
            disabled={loading || monthlyData.staffList.length === 0}
      ...
      <div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-2xs">
        ...
        ) : monthlyData.staffList.length === 0 ? (
          <div className="w-full py-12 text-center text-xs text-slate-400">No staff attendance records found for {selectedMonth}.</div>
        ) : (
          <div className="w-full overflow-x-auto">
            <table className="w-full border-collapse text-left text-xs">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200 text-[10px] font-semibold text-slate-500 uppercase">
                  <th className="px-3 py-2 sticky left-0 bg-slate-50 z-10 border-r border-slate-200">Staff Name</th>
                  <th className="px-2 py-2 border-r border-slate-200">Role</th>
                  {daysArray.map((day) => (
              ...
              <tbody className="divide-y divide-slate-100">
                {monthlyData.staffList.map((staff) => {
                  const staffLogs = monthlyData.attendanceLogs.filter((l) => l.staff_id === staff.staff_id);
                  let p = 0, lCount = 0, a = 0, v = 0;

                  return (
                    <tr key={staff.staff_id} className="hover:bg-slate-50/50 transition-colors">
                      ...
                      <td className="px-2 py-2 whitespace-nowrap text-[11px] text-slate-500 capitalize border-r border-slate-200">
                        {staff.role}
                      </td>
