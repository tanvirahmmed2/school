...
const AdminCurrentExamsPage = () => {
  ...
  const fetchExams = async () => {
    try {
      const response = await axios.get('/api/exams?status=current');
      ...
      toast.error(error.response?.data?.error || error.message || 'Failed to fetch active exams.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDeleteExam = async (id, examName) => {
    const confirm = window.confirm(
      `Are you sure you want to delete the exam routine for "${examName}"? All scheduled dates for classes will be lost.`
    );
    if (!confirm) return;
    ...
    <div className="w-full flex flex-col gap-6 animate-fade-up">
      ...
      <div className="flex flex-col gap-1">
        <h1 className="text-2xl font-bold text-slate-800 tracking-tight flex items-center gap-2">
          <FiCalendar className="text-blue-600" /> Current Active Exams
        </h1>
        <p className="text-sm text-slate-500">
          View, manage, edit, or delete active examination routines and routines.
        </p>
      </div>
      ...
      {editExamId ? (
        <div className="flex flex-col gap-4">
          <div className="flex justify-between items-center bg-slate-50 p-4 rounded-2xl border border-slate-100">
            <span className="text-sm font-semibold text-slate-650 text-slate-600">Editing Routine</span>
            <button
        ...
        <div className="w-full py-16 flex flex-col items-center justify-center gap-3">
          ...
          <span className="text-sm font-semibold text-slate-400">Loading active exams...</span>
        </div>
      ) : exams.length === 0 ? (
        <div className="w-full py-16 flex flex-col items-center justify-center text-center px-4 bg-white border border-slate-100 rounded-3xl shadow-[0_10px_40px_rgba(0,0,0,0.02)]">
          ...
          <h3 className="text-sm font-bold text-slate-600">No Current Active Exams</h3>
          <p className="text-xs text-slate-400 mt-1 max-w-[240px]">
            There are no exams currently set as active. Create or toggle routines via the "New Exam Routine" panel.
          </p>
        </div>
      ) : (
                ...
                <div className="px-6 py-5 border-b border-slate-100 bg-slate-50/50 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="p-2 bg-red-50 hover:bg-red-100 text-red-650 text-red-600 rounded-xl transition-colors cursor-pointer inline-flex items-center justify-center"
                ...
                <div className="p-6">
                  ...
                  {schedulesList.length === 0 ? (
                    <div className="text-center py-6 border border-dashed border-slate-150 rounded-2xl">
                      <p className="text-xs text-slate-400">No schedules mapped for this active exam routine yet.</p>
                    </div>
                  ) : (
...
export default AdminCurrentExamsPage;
