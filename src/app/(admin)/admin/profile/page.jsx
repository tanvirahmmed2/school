...
const AdminExperienceForm = ({ initial = {}, onSave, onCancel, loading }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-slate-50 border border-slate-200 rounded-2xl p-4 space-y-3">
      ...
      <label className="flex items-center gap-2 cursor-pointer">
        ...
        <span className="text-xs text-slate-600 font-medium">Currently in this role</span>
      </label>
      <div className="flex flex-col gap-1">
...
const AdminProfilePage = () => {
  ...
  const [profileData, setProfileData] = useState({
    name: '', number: '', address: '', date_of_birth: '', nationality: '', blood_group: '', gender: '', nid_number: '', bio: ''
  });
  ...
  const fetchProfile = async () => {
    try {
      ...
      const adminData = data.paylod?.admin;
      if (adminData) {
        ...
        setProfileData({
          ...
          date_of_birth: adminData.date_of_birth ? adminData.date_of_birth.split('T')[0] : '',
          nationality: adminData.nationality || '',
          blood_group: adminData.blood_group || '',
          gender: adminData.gender || '',
          nid_number: adminData.nid_number || '',
          bio: adminData.bio || ''
        });
        setIsTwoFactorEnabled(adminData.is_two_factor_enabled !== false);
        setExperiences(adminData.experiences || []);
      }
    } catch (error) {
  ...
  const handlePhotoUpload = (e) => {
    ...
    reader.onloadend = async () => {
      ...
      try {
        const res = await axios.put('/api/admin/profile', { image: reader.result });
        toast.dismiss(toastId); toast.success('Profile picture updated!');
        setProfile(res.data.paylod.admin);
  ...
  const handleUpdateProfile = async (e) => {
    ...
    try {
      ...
      if (data.paylod?.admin) setProfile(data.paylod.admin);
      setIsEditing(false);
    } catch (err) {
  ...
  const handleToggle2FA = async (newValue) => {
    ...
    try {
      ...
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || data.message || 'Failed to update 2FA status.');
  ...
  const handleExpSave = async (formData) => {
    ...
    try {
      if (formData.id) {
        const res = await axios.put('/api/admin/experiences', formData);
        setExperiences((prev) => prev.map((e) => e.id === formData.id ? res.data.paylod.experience : e));
        toast.success('Experience updated.');
      } else {
        const res = await axios.post('/api/admin/experiences', formData);
  ...
  const handleExpDelete = async (id) => {
    ...
    try {
      await axios.delete(`/api/admin/experiences?id=${id}`);
  ...
  if (loading) {
    return (
      <div className="w-full py-20 flex flex-col items-center justify-center gap-3">
        ...
        <span className="text-sm font-semibold text-slate-400">Loading administrator profile...</span>
      </div>
    );
  }
    ...
    <div className="w-full flex flex-col gap-6 animate-fade-up max-w-5xl mx-auto pb-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white border border-slate-100 p-6 rounded-3xl shadow-[0_10px_40px_rgba(0,0,0,0.02)]">
        <div className="flex items-center gap-4">
          <div className="relative group w-16 h-16 rounded-2xl bg-primary/10 text-primary border border-primary/20 flex items-center justify-center font-bold text-2xl shadow-xs overflow-hidden shrink-0">
            {profile?.image ? (
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="inline-flex items-center gap-1 text-xs font-bold text-emerald-700 bg-emerald-50 px-2.5 py-0.5 rounded-full border border-emerald-100">
                <FiCheckCircle className="text-xs" /> System Admin
              </span>
            </div>
        ...
        <button onClick={() => setIsEditing(!isEditing)}
          className="flex items-center gap-1.5 px-4 py-2 text-xs font-semibold text-white bg-primary rounded-xl hover:opacity-90 transition-opacity shrink-0">
      ...
      {isEditing ? (
        <form onSubmit={handleUpdateProfile} className="bg-white border border-slate-100 p-6 rounded-3xl shadow-xs space-y-4">
          <h2 className="text-sm font-bold text-slate-800 uppercase tracking-wider border-b border-slate-100 pb-2">Edit Admin Details</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {[
              ...
              <div key={name} className="flex flex-col gap-1">
                ...
                <input type={type} name={name} value={profileData[name]} onChange={(e) => setProfileData({ ...profileData, [name]: e.target.value })}
                  className="border border-slate-200 rounded-xl px-3 py-2 text-xs text-slate-800 focus:outline-none focus:ring-2 focus:ring-primary/30" />
              </div>
            ))}
            <div className="flex flex-col gap-1">
            ...
            <div className="sm:col-span-2 flex flex-col gap-1">
              ...
              <textarea name="bio" rows={3} value={profileData.bio} onChange={(e) => setProfileData({ ...profileData, bio: e.target.value })}
                className="border border-slate-200 rounded-xl px-3 py-2 text-xs text-slate-800 focus:outline-none focus:ring-2 focus:ring-primary/30 resize-none" />
            </div>
          </div>
          <button type="submit" disabled={updatingProfile} className="flex items-center gap-1.5 px-5 py-2.5 text-xs font-semibold text-white bg-primary rounded-xl hover:opacity-90 transition-opacity disabled:opacity-60">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {[
              ...
              <div key={label} className="flex items-start gap-3 p-3 bg-slate-50/70 rounded-2xl">
                <Icon className="text-primary text-sm mt-0.5 shrink-0" />
                <div>
            ...
            {profile?.bio && (
              <div className="sm:col-span-2 flex items-start gap-3 p-3 bg-slate-50/70 rounded-2xl">
                <FiUser className="text-primary text-sm mt-0.5 shrink-0" />
                <div>
      ...
      <div className="bg-white border border-slate-100 p-6 rounded-3xl shadow-xs flex items-center justify-between gap-4">
        <div>
          <h2 className="text-sm font-bold text-slate-800 flex items-center gap-1.5"><FiShield className="text-primary" /> Two-Factor Authentication</h2>
          <p className="text-xs text-slate-400 mt-0.5">{isTwoFactorEnabled ? '2FA is active — Mandatory for admin access.' : '2FA is disabled.'}</p>
        </div>
      ...
      <form onSubmit={handleUpdatePassword} className="bg-white border border-slate-100 p-6 rounded-3xl shadow-xs space-y-4">
        <h2 className="text-xs font-bold text-slate-700 uppercase tracking-wider border-b border-slate-100 pb-2 flex items-center gap-1.5"><FiLock className="text-primary" /> Update Password</h2>
        ...
        <button type="submit" disabled={updatingPassword} className="flex items-center gap-1.5 px-5 py-2.5 text-xs font-semibold text-white bg-primary rounded-xl hover:opacity-90 transition-opacity disabled:opacity-60">
      ...
      <div className="bg-white border border-slate-100 p-6 rounded-3xl shadow-xs space-y-4">
        <div className="flex items-center justify-between border-b border-slate-100 pb-3">
          <h2 className="text-xs font-bold text-slate-700 uppercase tracking-wider flex items-center gap-1.5"><FiStar className="text-primary" /> Administrative & Professional Experience ({experiences.length})</h2>
          <button onClick={() => { setShowExpForm(!showExpForm); setEditingExp(null); }}
            className="flex items-center gap-1 px-3 py-1.5 bg-primary text-white text-[10px] font-semibold rounded-xl hover:opacity-90 transition-opacity">
            <FiPlus /> Add
          </button>
        </div>

        {showExpForm && !editingExp && (
          <AdminExperienceForm onSave={handleExpSave} onCancel={() => setShowExpForm(false)} loading={expLoading} />
        )}
            ...
            {experiences.map((exp) => (
              <div key={exp.id}>
                {editingExp?.id === exp.id ? (
                  <AdminExperienceForm initial={exp} onSave={handleExpSave} onCancel={() => setEditingExp(null)} loading={expLoading} />
                ) : (
