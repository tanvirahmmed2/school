...
import {
  ...
  FiShield, FiSave, FiKey, FiCalendar, FiClock, FiToggleLeft, FiToggleRight
} from 'react-icons/fi';
...
const AdminProfilePage = () => {
  ...
  const handleUpdateProfile = async (e) => {
    ...
    try {
      ...
      if (data.paylod?.admin) {
        setProfile(data.paylod.admin);
      }
    } catch (err) {
  ...
  const handleToggle2FA = async (newValue) => {
    ...
    try {
      ...
      const data = await response.json();
      if (!response.ok) {
        throw new Error(data.error || data.message || 'Failed to update 2FA status.');
      }
    ...
    <div className="w-full flex flex-col gap-6 animate-fade-up max-w-6xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white border border-slate-100 p-6 rounded-3xl shadow-[0_10px_40px_rgba(0,0,0,0.02)]">
        <div className="flex items-center gap-4">
          <div className="w-16 h-16 rounded-2xl bg-primary/10 text-primary border border-primary/20 flex items-center justify-center font-bold text-2xl shadow-xs">
            <FiUser />
          </div>
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="inline-flex items-center gap-1 text-xs font-bold text-emerald-700 bg-emerald-50 px-2.5 py-0.5 rounded-full border border-emerald-100">
                <FiCheckCircle className="text-xs" /> Active Admin
              </span>
            </div>
            <p className="text-xs text-slate-400 mt-1 flex items-center gap-3">
              <span className="flex items-center gap-1"><FiMail /> {profile?.email}</span>
              <span>•</span>
              <span className="flex items-center gap-1"><FiShield /> Admin ID: #{profile?.id}</span>
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 border text-xs font-semibold rounded-xl ${
            ...
            <FiShield className={isTwoFactorEnabled ? 'text-emerald-600' : 'text-amber-600'} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 flex flex-col gap-6">
          <div className="bg-white border border-slate-100 rounded-3xl p-6 md:p-8 shadow-[0_10px_40px_rgba(0,0,0,0.02)]">
            <h2 className="text-base font-bold text-slate-800 mb-6 flex items-center gap-2 border-b border-slate-100 pb-4">
              <FiUser className="text-primary text-lg" /> Personal Details
            </h2>
            ...
            <form onSubmit={handleUpdateProfile} className="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <div className="flex flex-col gap-1.5 md:col-span-2">
                ...
                <textarea
                  ...
                  disabled={updatingProfile}
                  className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-900 placeholder:text-slate-400 outline-none transition-all focus:bg-white focus:border-primary focus:ring-4 focus:ring-primary/5 resize-none"
                />
              </div>
              ...
              <div className="flex justify-end md:col-span-2 mt-2">
                <button
                  type="submit"
                  disabled={updatingProfile}
                  className="px-5 py-2.5 bg-primary hover:bg-primary-dark text-white rounded-xl text-sm font-semibold transition-all duration-150 flex items-center gap-2 disabled:opacity-50 cursor-pointer shadow-xs"
                >
                  {updatingProfile ? (
          ...
          <div className="bg-white border border-slate-100 rounded-3xl p-6 md:p-8 shadow-[0_10px_40px_rgba(0,0,0,0.02)] flex flex-col gap-4">
            ...
            <p className="text-xs text-slate-500 leading-relaxed">
              When enabled, signing into your administrator account will require entering a one-time passcode sent directly to your registered email address ({email}).
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="bg-white border border-slate-100 rounded-3xl p-6 shadow-[0_10px_40px_rgba(0,0,0,0.02)]">
            <h2 className="text-base font-bold text-slate-800 mb-6 flex items-center gap-2 border-b border-slate-100 pb-4">
              <FiKey className="text-primary text-lg" /> Security & Password
            </h2>
          ...
          <div className="bg-slate-900 text-white rounded-3xl p-6 shadow-sm flex flex-col gap-4">
            <h3 className="text-sm font-bold flex items-center gap-2 text-slate-200">
              <FiShield className="text-primary text-base" /> Account Metadata
            </h3>
            ...
            <div className="flex flex-col gap-3 text-xs text-slate-300 divide-y divide-slate-800">
              ...
              <div className="flex items-center justify-between pt-3">
                ...
                <span className={`font-semibold ${isTwoFactorEnabled ? 'text-emerald-400' : 'text-amber-400'}`}>
